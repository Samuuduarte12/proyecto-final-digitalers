...
const ProductoCard = ({ product ,productosDestacados}) => {
    ...
    return (
        <div className="group relative overflow-hidden rounded-2xl bg-[#0a121f]/50 backdrop-blur-sm border border-[#202938]/50 transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl hover:shadow-[#00afb3]/20 min-h-[480px] lg:min-h-[100px] flex flex-col justify-between">
            {productosDestacados &&
                <div className="absolute top-4 right-4 z-10 bg-linear-to-r from-[#00afb3] to-[#e65fde] text-[#f3faff] px-3 py-1 rounded-full text-xs font-semibold shadow-lg">
                    Destacado
                </div>
            }
            ...
            <div className="p-6 space-y-3">
                ...
                <Link to={`/detalleProducto/${product.id}`} className="flex items-start justify-between gap-2">
                    <h3 className="font-semibold text-lg text-[#f3faff] leading-tight line-clamp-2">
                        {product.title}
                    </h3>
                </Link>

                <p className="text-sm text-[#85919a] line-clamp-2">
                    {product.description}
                </p>
                ...
                <div className="flex items-center justify-between pt-2">
                    ...
                    <button
                        ...
                        size="sm"
                        className="bg-linear-to-r cursor-pointer flex items-center px-2 lg:px-2 py-1 lg:text-sm rounded-md from-[#00afb3] to-[#e65fde] hover:opacity-90 transition-opacity shadow-lg shadow-[#00afb3]/30"
                    >
